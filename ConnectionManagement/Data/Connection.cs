...
using System.Linq;
using System.IO;
using ConnectionManagement.Configuration;
...
namespace ConnectionManagement.Data
{
    ...
    public class Connection : System.IDisposable
    {
        ...
        //private SqlClientPermission pSql = new SqlClientPermission(PermissionState.Unrestricted);
        ...
        {
            //pSql.Assert();
            this.LeerCadenaDeConexion();
            this.conexion = new SqlConnection(_ConnectionString);
            comando = this.ConexionSQL.CreateCommand();
            comando.Connection = this.ConexionSQL;
        }
         public void LeerCadenaDeConexion()
        {
            ...
            var cadena = isDevelopment ? "DefaultConnection" : "produccion";

            _ConnectionString = configuration["ConnectionStrings:"+cadena];
        ...
        public async Task<bool> CerrarConexionAsync()
        {
            if (comando.Transaction == null)
            {
                if (conexion.State != ConnectionState.Closed)
        ...
        //'''</remarks>
        public int EjecutarNonQuery(string sentencia, CommandType tipoComando = CommandType.Text)
        {
            if (AbrirConexion())
            {
        ...
        public async Task<int> EjecutarNonQueryAsync(string sentencia, CommandType tipoComando = CommandType.Text)
        {
            if (await AbrirConexionAsync())
            {
                comando.CommandType = tipoComando;
                comando.CommandText = sentencia;
                try
                ...
                {
                    await this.CerrarConexionAsync();
                }
            }
            return -1;
        }
        ...
        //''' </summary>
        public InfoObjetoConexion ObtenerInformacionDeConexion(IConfiguration cadenaConexion)
        {
            InfoObjetoConexion infoConexion = new InfoObjetoConexion();
            SqlConnectionStringBuilder builder = new SqlConnectionStringBuilder(cadenaConexion.GetConnectionString("DefaultConnection"));
            infoConexion.CadenaConexion = builder.ConnectionString;
            infoConexion.NombreBaseDatos = builder.InitialCatalog;
            infoConexion.NombreServidor = builder.DataSource;
            infoConexion.TiempoEspera = builder.ConnectTimeout;
            infoConexion.NombreUsuario = builder.UserID;
            infoConexion.Password = builder.Password;
            builder.Clear();
            return infoConexion;
        }
        #endregion
        ...
        public DataSet BuildDataset(SqlCommand sqlInstruction, IConfiguration cadenaConexion)
        {
            localConnection = new SqlConnection(cadenaConexion.GetConnectionString("DefaultConnection"));
            localConnection.Open();

            sqlInstruction.Connection = localConnection;

            SqlDataAdapter sqlAdapter = new SqlDataAdapter();
            sqlAdapter.SelectCommand = sqlInstruction;

            DataSet datasetToReturn = new DataSet();

            sqlAdapter.Fill(datasetToReturn);
        ...
        /// <summary>
        /// Método que crea y retorna un datatable a partir del resulato de un procedimiento almacenado o instrucción sql
        /// </summary>
        /// <param name="sqlInstruction"></param>
        /// <returns>Retorna un Datatable</returns>
        public DataTable BuildDatatable(SqlCommand sqlInstruction)
        {
            localConnection.Open();
            ...
            sqlAdapter.Fill(datatableToReturn);
        ...
        public List<T> MapearClase<T>(string RawSql, CommandType TipoComando = CommandType.StoredProcedure) where T : class, new()
        {
            List<T> Data = new List<T>();
            if (AbrirConexion())
            {
                comando.CommandType = TipoComando;
                comando.CommandText = RawSql;
                try
                {
                    ...
                    while (Reader.Read())
                    {
                        ...
                        {
                            if (Reader[x.Name] != DBNull.Value && Reader[x.Name] != null)
                                x.SetValue(Val, Convert.ChangeType(Reader[x.Name], x.PropertyType));
                        });
                        Data.Add(Val);
                    }
                }
        ...
        public SqlParameterCollection ObtenerParametros()
        {
            return comando.Parameters;
        }

        public async Task<List<T>> MapearClaseAsync<T>(string RawSql, CommandType TipoComando = CommandType.StoredProcedure) where T : class, new()
        {
            List<T> Data = new List<T>();
            if (await AbrirConexionAsync())
            {
                    ...
                    while (await Reader.ReadAsync())
                    {
                        T Val = new T();
                            ...
                            typeof(T).GetProperties().ToList().ForEach(x =>
                            {
                                try
                                {
                                    if (Reader[x.Name] != null && Reader[x.Name] != DBNull.Value)
                                        x.SetValue(Val, Convert.ChangeType(Reader[x.Name], x.PropertyType));
                                }
                                catch (Exception ex)
                                { }
        ...
        public async Task<T> MapearObjetoAsync<T>(string RawSql, CommandType TipoComando = CommandType.StoredProcedure) where T : class, new()
        {
            T Data = null;
            if (await AbrirConexionAsync())
            {
